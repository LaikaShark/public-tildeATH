...
def test_block_comments_do_not_nest():
    # first */ closes regardless of inner /*; only 'c' remains
...
def test_int_literal_over_int64_becomes_bigint():
    # integer literal too large for int64 is a bignum literal, not an error
...
def test_dot_disambiguation_slice_vs_float_vs_die():
    # 1..3 is INT DOTDOT INT (slice range), not a float
    toks = tokenize("1..3")
    ...
    # 5.die is INT then .DIE; '.' does not start a fraction
    toks = tokenize("5.die")
    assert toks[0].kind is TokenKind.INT
...
def test_e_after_int_not_swallowed():
    # 1exit is INT(1) then identifier exit, not a bad exponent
    toks = tokenize("1exit")
    assert toks[0].kind is TokenKind.INT
...
def test_lone_minus_without_digits_errors():
    # bare '-' outside a number has no meaning
    with pytest.raises(LexError):
        tokenize("-foo")
...
def test_dotdot_in_range_subscript():
    ...
    # confirm DOTDOT appears between the two idents
...
def test_print2_is_no_longer_a_keyword(spelling):
    # PRINT2 removed in favor of `print $var`; now an ordinary identifier
    toks = tokenize(spelling)
...
def test_print_payload_can_be_empty():
    # empty payload emits no literal part, just keyword and ';'
    toks = tokenize("print ;")
...
def test_print_payload_interpolation_stops_at_nonident():
    # $W. — name is just W; '.' is literal text
