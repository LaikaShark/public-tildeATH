...
def test_renderer_caret_alignment():
    ...
    # format "  2 | source"; caret in same after-pipe region
    pipe_idx = caret_line.index("|")
    after_pipe = caret_line[pipe_idx + 1 :]
    # caret preceded by col-1 spaces plus the space after pipe
    assert after_pipe == "   ^"
...
def test_renderer_handles_line_past_eof():
    # line past end clamps to last line; still renders snippet
    out = render_diagnostic(
        "error", "msg", "p.ath", "one\ntwo\n", line=99, col=1
    )
    assert "two" in out
...
def test_e2e_importf_missing_file_renders_against_main_source(tmp_path):
    ...
    # caret renders against main.ath where the importf is
...
def test_edit_distance_basic():
    ...
    # insertion
    assert edit_distance("printt", "print") == 1
    # transposition (Damerau)
    assert edit_distance("improt", "import") == 1
    # transposition
    assert edit_distance("nmae", "name") == 1
...
def test_closest_picks_near_and_skips_far():
    ...
    assert closest("add", cands, fold=True) is None
...
def test_unknown_function_suggests_registered_name():
    # loader normally populates function table; pass directly here
    prog = parse("import number 1 as N;\nADXD [N, N] R;")
    with pytest.raises(SemaError) as ei:
        analyze(prog, {"ADD": object()})
    # casing echoed
    assert ei.value.help == "did you mean 'ADD'?"
