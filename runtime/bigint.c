// Arbitrary-precision signed integer. Sign-magnitude, base-2^32 little-endian
// limbs. Schoolbook algorithms, correctness over speed. Allocate-and-leak.
...
// Strip leading zero limbs; zero magnitude forces sign 0. Never sets nonzero
// sign — magnitude builders set sign before calling.
...
// |a| + |b|, result sign +1 (caller adjusts).
...
ath_bigint *ath_big_from_i64(int64_t v) {
    ...
    if (!b) return NULL;
    uint64_t mag;
    // -(uint64_t)INT64_MIN well-defined
    if (v < 0)      { b->sign = -1; mag = -(uint64_t)v; }
...
ath_bigint *ath_big_sub(const ath_bigint *a, const ath_bigint *b) {
    // read-only negated view
    ath_bigint nb = { -b->sign, b->len, b->limbs };
...
int ath_big_divmod(const ath_bigint *a, const ath_bigint *b,
    ...
    // truncated: remainder takes a's sign
...
// Divide magnitude in place by small divisor, return remainder.
...
char *ath_big_to_decimal(const ath_bigint *a) {
    ...
    size_t tn = n;
    // each limb < 2^32 < 10^10, so n limbs hold < 10^(10n); n*10 digits safe bound
    size_t cap = n * 10 + 3;
