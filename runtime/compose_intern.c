/* Composition discipline: INTERN (SPEC §4.4.3, §5.2). ath_compose is
 * hash-consed by the raw pointer-pair (left, right): two calls with the
 * same operands return the same object, whether alive or dead. Killing
 * an interned composite affects every variable that ever observed it. */
...
#define ATH_INTERN_TABLE_SIZE 4096   /* power of 2 */
