...
this tutorial introduces `~ATH`. `SPEC.md` is the technical reference
...
source. a `$VAR` marker in the payload coerces the object bound to `VAR` to a string

six escape sequences are recognized in the payload:
...
. unknown escapes are also a compile-time error.
...
the top level terminates the program
...
two rules govern these fields:

1. `alive` transitions from true to false at most once and never back
2. `left` and `right` are set together on first decomposition
...
every activation begins with two predefined identifiers:
...
bound to the argument object the caller provided.
...
case-sensitive, `this` and `Null` are distinct identifiers
...
unbound name yields `NULL` at runtime. read of a variable that has
...
runtime looks up in the lifetime library. if no library entry
...
`BUILTIN` or `NUMBER`; those dispatch to separate forms
...
its left and right halves. on the first decomposition of an
...
halves, and binds `V` to it. the exact identity of the
...
killing an object sets its `alive` field to false. the
...
`THIS.DIE(RET);`, this is how a function returns a value

when the killed object owns a file (see `READ` below), the runtime
...
loop exits when `ath_is_alive(V)` returns false
...
exit a loop without killing the object originally bound to `V`
...
`loop N { body }`runs `body` exactly `N.value` times; a
...
`every N { body }`runs `body`, sleeps `N.value` ms, and
...
property of the loop, never of an object — an object cannot be revived
...
exit from a `BRANCH`, regardless of which arm ran
...
from `V`'s current binding at clone time:
...
`owns_path` flag. `W` is independent: killing one of `V`
...
`IMPORTF "PATH" AS NAME;`is a compile-time directive: it
...
existing `<stem>.ath` wins. this form is used to bring
in standard-library shims
...
the C symbol `SYMBOL`. the C function must have the
...
and calls `ADD [X, Y]R;` like any other function
...
`alive` bit. the payload is taggedwith a `num_kind` that says whether it
...
exponent (`3.14`, `1e6`, `2.5e-3`) gives a FLOAT payload. the
object is bound to the name. an integer literal must fit
...
| `TO_STRING [N, _]S;`   | decimal string of `N`|
...
each call returns a fresh object that is alive on success and born-dead on failure. failure conditionsinclude:
...
### 11.2 Floats and the numeric tower
...
an integer literal too large for int64 becomes a bignum:
...
## 12. Lifetime extensions

an object may carry up to five optional lifetime conditions in
addition to its explicit `.DIE`-driven mortality. on every
...
all death is one-way; a dead object never becomes alive

### 12.1 The lifetime library
...
the full table is in the speca representative selection:
...
take ownership of the file. the owning equivalent is `READ`

### 12.5 Watching processes and file changes

two more `WATCH` forms tie liveness to external state:
...
negated comparison can be combined with `AND` and `OR`
without needing a NOT-of-verdict construct (which would conflict with
mortality)

`~ATH(!V)`provides the same inversion at a single observation
...
### 13.1 Lifetime inheritance

a true verdict installs a dependency on both operands via the runtime
helper `ath_inherit_lifetime`. the runtime records the two
...
### 13.3 Combining verdicts: AND, OR
...
`AND` uses the conjunctive dependency machinery: the
...
`TEXT` statement, and are written out through `print $VAR`
interpolation, `WRITE`, or `APPEND`

### 14.1 `INPUT` and `print $VAR`
...
remaining bytes as a cons-list, and binds `VAR` to it. on
...
`PRINT` does double duty. its payload is raw literal text,
...
with `ENTANGLE` just like `REPLACE`. 
...
#### 14.2.2 Wrapping an atom in a string
...
#### 14.2.3 The compose-pair pattern
...
. killing `NEEDLE` or `REPLACEMENT` after the `REPLACE` call
...
its range endpoints, but the user never writes that
...
machinery: killing any of the source identifiers after the
`TEXT` statement runs invalidates `MSG` at the next observation

#### 14.3.3 Single-IDENT case
...
#### 14.3.4 What `TEXT` is and is not
...
a family of stdlib shims folds and slices number lists:
...
payload: they fold the `AND`/`OR` verdictsover the list, so the
...
fixed folds plus `SPLIT`/`JOIN`for strings
...
. if `N` is `NULL`, dead, lacks a payload, or carries a
...
`N.value` milliseconds from now, and binds `T` to it. `T`
...
seven statements interact with the filesystem:
...
- `LISTDIR PATH AS VAR;` — list directory entries
- `EXISTS PATH AS VAR;` — test whether a path exists

`PATH` in all of these may be a **string literal** (`"data.txt"`) or a
**variable** holding a string (`FNAME`). this lets programs build paths
at runtime with `TEXT` and pass them to file operations:

```ath
TEXT "output/" NAME ".txt" AS PATH;
WRITE CONTENT TO PATH AS OK;
```

`WATCH "PATH" AS V;`is also file-related but is observational
...
`READ PATH AS VAR;`opens `PATH` for reading (relative to
...
when `PATH` is a variable, the runtime coerces it to bytes. this lets
you read files whose names are computed at runtime:

```ath
TEXT "data/" FILENAME AS P;
READ P AS CONTENTS;
```
...
`BRANCH`, the runtime calls `unlink(PATH)` *before* flipping
the alive bit. the example above therefore **deletes
...
to release the file without deleting it, use `CLOSE`
...
`WRITE SRC TO PATH [AS VERDICT];`opens `PATH` for
...
identical to `WRITE`except the file is opened for appending.
`PATH` may be a string literal or variable, same as `WRITE`.
...
`CLOSE VAR;`clears `VAR`'s `owns_path` flag, then sets its
...
`MKDIR PATH;` creates a directory at `PATH`, including
intermediate parents — `MKDIR "a/b/c";` creates `a/`, `a/b/`, and
`a/b/c/` if none exist. if the directory already exists, the statement
succeeds silently. on failure (permission denied, invalid path) it is
a no-op. there is no return value

```ath
TEXT ".ant/chambers/" ID AS DIR;
MKDIR DIR;
```
...
```ath
LISTDIR "src" AS FILES;
~ATH(FILES) {
    BIFURCATE FILES[NAME, FILES];
    PRINT $NAME;
}
```

`LISTDIR PATH AS VAR;` lists the entries of the directory
at `PATH`, excluding `.` and `..`, and binds `VAR` to a cons-list of
string names. each element is a filename (not a full path). on failure
or if the directory is empty, `VAR` is bound to `NULL`

the list is walked exactly like any other cons-list: decompose the
head off in a `~ATH` loop. entries are in filesystem order (whatever
`readdir` returns)

```ath
TEXT ".ant/refs" AS RDIR;
LISTDIR RDIR AS REFS;
```

### 16.8 `EXISTS`

```ath
EXISTS "config.txt" AS E;
BRANCH(E) {
    PRINT found;
}
ELSE {
...
`EXISTS PATH AS VAR;` tests whether `PATH` exists in the
filesystem — files, directories, symlinks, anything `stat` can see.
`VAR` is bound to a verdict: alive if the path exists, dead otherwise.
the verdict is a snapshot, not a live observation (unlike `WATCH`)

```ath
TEXT ".ant/refs/" BRANCH AS REF;
EXISTS REF AS OK;
CLONE OK AS OKC;
BRANCH(!OKC) {
...
- syntactic: any deviation from the grammar
...
crash, abort, or print a diagnostic. every operation is
...
they are the ordinary liveness modelapplied to mailboxes and
...
- `CONNECT "host" PORT AS C;` (or `CONNECT HOST PORT AS C;`, or
  `CONNECT "unix:/path" AS C;`) — dial out. the host may be a string
  literal or a variable holding a string.
...
- `SPEC.md` is the authoritative reference
