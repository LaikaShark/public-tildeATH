...
# `every` is infinite; cap it so the REPL never hangs. Generous but bounded.
_EVERY_CAP = 1000
...
class Evaluator:
    ...
    def run(self, stmts):
        ...
        try:
            ...
            # Top-level activation ended; reset THIS so the session continues.
    ...
    def _eval(self, s):
        ...
        if m is None:
            raise ReplError(
                f"{type(s).__name__} is not supported in the REPL"
            )
        m(self, s)

    # -- statements ----------------------------------------------------------
    def _import(self, s: ImportStmt):
        if s.var in self.env:           # idempotent (§4.4.1)
            return
    ...
    def _ath_loop(self, s: AthLoop):
        while True:
            ...
            if not cond:
                break
            self._run_body(s.body)   # in-body THIS.DIE() propagates past EXECUTE
    ...
    def _every(self, s: EveryStmt):
        # Infinite in a compiled program; bounded here so the REPL never hangs.
    ...
    def _funcall_compose(self, s: FuncCallComposeArg):
        l, r = self._read(s.left), self._read(s.right)
    ...
    def _run_user(self, program, args):
        ...
        try:
            for st in program.statements:
                self._eval(st)
            return self.rt.NULL          # fell off the end
    ...
    def _subscript(self, s: SubscriptStmt):
        obj = self.rt.lib.ath_index(self._read(s.source), self._read(s.index))
        self._write(s.target, obj)

    def _slice(self, s: SliceStmt):
        i, j = self._read(s.start), self._read(s.end)
    ...
    def _branch(self, s: BranchStmt):
        ...
        # Consume the subject (§4.4.17); re-read in case a body rebound it.
        self.rt.lib.ath_die(self._read(s.var))
