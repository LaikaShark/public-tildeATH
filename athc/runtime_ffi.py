...
# --- struct layout (must mirror runtime/ath_runtime.h ath_obj) -------------
# Only the REPL's introspection (to_text / inspect) reads the struct; all
# *evaluation* goes through ABI functions, so a layout drift would degrade
# only inspection, never correctness.
...
def _default_lib(mode: str) -> Path:
    return Path(__file__).resolve().parent.parent / "runtime" / f"libath_{mode}.so"
...
class AthRuntime:
    ...
    # name -> (restype, [argtypes]) for the non-generic entry points. Every
    # other `ath_obj*(ath_obj*, ath_obj*)` builtin is bound on demand by
    # `builtin()`.
    _SIGS = {
        "ath_alloc_alive": (ptr, []),
    ...
    def __init__(self, mode: str = "fresh"):
        ...
        if not path.exists():
            raise RuntimeNotBuilt(
                f"runtime shared library not found at {path}; run 'make runtime'"
            )
        ...
        # The dead singleton (extern ath_obj *ath_NULL).
        self.NULL = ctypes.c_void_p.in_dll(self.lib, "ath_NULL").value

    # -- generic builtin binding --------------------------------------------
    ...
    # -- introspection (struct reads; see note above) -----------------------
