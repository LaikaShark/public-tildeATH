...
def _add_operand(names: set, op) -> None:
    # Only bound-name operands need a stack slot; inline-literal Operands
    # are materialized at the call site and never read from a slot.
...
def _collect_names(stmts, names: set) -> None:
    for s in stmts:
        if isinstance(s, ImportStmt):
            ...
            _add_operand(names, s.left)
            _add_operand(names, s.right)
            names.add(s.target)
        elif isinstance(s, FuncCallDecomposeRet):
            _add_operand(names, s.arg)
            names.add(s.left)
            names.add(s.right)
        elif isinstance(s, SubscriptStmt):
            names.add(s.source)
            _add_operand(names, s.index)
            names.add(s.target)
        elif isinstance(s, SliceStmt):
            names.add(s.source)
            _add_operand(names, s.start)
            _add_operand(names, s.end)
            names.add(s.target)
            ...
            # Target is a write, each IDENT part a read; all need slots
            names.add(s.target)
            for part in s.parts:
                if part.kind == "ident":
                    names.add(part.value)
...
def _contains_spawn(stmts) -> bool:
    """True iff any statement (recursively, through loop/branch bodies) is a SPAWN. Used to
    emit ath_scheduler_drain() only for programs that actually start actors."""
    for s in stmts:
        if isinstance(s, SpawnStmt):
            return True
        for attr in ("body", "then_body", "else_body"):
            sub = getattr(s, attr, None)
            if sub and _contains_spawn(sub):
                return True
    return False
...
class Codegen:
    def __init__(
        ...
        # Newline-free primitive: one `print` emits each part then a single trailing line feed
        ...
        # Cooperative scheduler (runtime/scheduler.c). User functions have type
        # ath_obj*(ath_obj*); ath_spawn takes a pointer to one.
        self.user_fn_ty = ir.FunctionType(self.obj_ptr, [self.obj_ptr])
        user_fn_ptr = self.user_fn_ty.as_pointer()
        self.f_spawn = ir.Function(
            self.module,
            ir.FunctionType(self.obj_ptr, [user_fn_ptr, self.obj_ptr]),
            name="ath_spawn",
        )
        self.f_spawn_into = ir.Function(
            self.module,
            ir.FunctionType(self.obj_ptr, [user_fn_ptr, self.obj_ptr, self.obj_ptr]),
            name="ath_spawn_into",
        )
        self.f_send = ir.Function(
            self.module,
            ir.FunctionType(ir.VoidType(), [self.obj_ptr, self.obj_ptr]),
            name="ath_send",
        )
        self.f_recv = ir.Function(
            self.module, ir.FunctionType(self.obj_ptr, []), name="ath_recv"
        )
        self.f_recv_from = ir.Function(
            self.module,
            ir.FunctionType(self.obj_ptr, [self.obj_ptr]),
            name="ath_recv_from",
        )
        self.f_yield = ir.Function(
            self.module, ir.FunctionType(ir.VoidType(), []), name="ath_yield"
        )
        self.f_join_handle = ir.Function(
            self.module,
            ir.FunctionType(ir.VoidType(), [self.obj_ptr]),
            name="ath_join_handle",
        )
        ...
        # Per-file local builtin tables: name (lower) → ir.Function; populated in generate() before any FunctionEmitter runs
        ...
        # Pre-seed c-builtin table so user `import builtin ath_concat` reuses this declaration instead of duplicating
        ...
        # Register builtins per program; all use fixed (ath_obj*, ath_obj*) -> ath_obj* ABI
...
class FunctionEmitter:
    ...
    def emit(self) -> None:
        ...
        # Register user-defined library entries (--define-lifetime) before any other code, so first import sees them
        if self.is_main:
        ...
        # whether main falls off the end (here) or exits early via THIS.DIE() (in _emit_die).
        # Non-spawning programs emit nothing new and stay byte-identical to today.
        self._has_spawn = self.is_main and _contains_spawn(self.program.statements)

        self._emit_block(builder, self.program.statements)

        if self._has_spawn and not builder.block.is_terminated:
            builder.call(self.cg.f_scheduler_drain, [])
    ...
    def _emit_operand(self, builder: ir.IRBuilder, op) -> ir.Value:
        # A read-operand: a bound name loads its slot; an inline literal
        # materializes a fresh object via the same constructors used by
        # IMPORT NUMBER (_emit_import_number) and string literals.
        if isinstance(op, str):
            return self._read_var(builder, op)
        if op.kind == "string":
            return self._emit_string_literal(builder, op.value)
        if op.kind == "float":
            return builder.call(
                self.cg.f_alloc_float,
                [ir.Constant(ir.DoubleType(), float(op.value))],
            )
        if op.kind == "bignum":
            g = self.cg.make_cstring_global(op.value)
            zero = ir.Constant(self.cg.i32, 0)
            ptr = builder.gep(g, [zero, zero], inbounds=True)
            return builder.call(self.cg.f_alloc_bignum_decimal, [ptr])
        return builder.call(
            self.cg.f_alloc_number, [ir.Constant(self.cg.i64, op.value)]
        )
    ...
    def _emit_stmt(self, builder: ir.IRBuilder, stmt) -> None:
        if isinstance(stmt, ImportStmt):
            ...
            # declaration only; handled by Codegen at init time
            pass
            ...
            self._emit_spawn(builder, stmt)
        elif isinstance(stmt, SendStmt):
            self._emit_send(builder, stmt)
        elif isinstance(stmt, RecvStmt):
            self._emit_recv(builder, stmt)
        elif isinstance(stmt, YieldStmt):
            builder.call(self.cg.f_yield, [])
    ...
    def _emit_spawn(self, builder: ir.IRBuilder, stmt: SpawnStmt) -> None:
        fname = stmt.name.lower()
        fn = self.cg.user_fns.get(fname)
        if fn is None:
    ...
    def _emit_send(self, builder: ir.IRBuilder, stmt: SendStmt) -> None:
        msg = self._emit_operand(builder, stmt.message)
        dest = self._read_var(builder, stmt.dest)
        builder.call(self.cg.f_send, [dest, msg])

    def _emit_recv(self, builder: ir.IRBuilder, stmt: RecvStmt) -> None:
        if stmt.source is not None:
            src = self._read_var(builder, stmt.source)
            msg = builder.call(self.cg.f_recv_from, [src])
        else:
    ...
    def _emit_ath_loop(self, builder: ir.IRBuilder, stmt: AthLoop) -> None:
        ...
        # EXECUTE(F) postfix: on natural exit, call F with subject (dead for normal loop, alive for inverted); "NULL" is no-op; THIS.DIE() in body skips this
    ...
    def _emit_loop(self, builder: ir.IRBuilder, stmt: LoopStmt) -> None:
        # Counted loop over i64 phi; count is ath_count_of(N) clamped to >= 0
        fn = builder.function
        loop_id = self._loop_id
        self._loop_id += 1
        ...
        # Latch decrements and re-enters header; it's a CFG predecessor (phi lists it) even when body always terminates
    ...
    def _emit_every(self, builder: ir.IRBuilder, stmt: EveryStmt) -> None:
        # Run body, sleep N ms, forever; exits only when body terminates activation (THIS.DIE / signal); code after is unreachable
        fn = builder.function
        loop_id = self._loop_id
        self._loop_id += 1
    ...
    def _emit_die(self, builder: ir.IRBuilder, stmt: DieStmt) -> None:
        # Read arg first so THIS.DIE(THIS) returns still-live THIS pointer before the kill
        ...
        if stmt.var == "THIS":
            # Top-level THIS.DIE() means shutdown: let spawned actors finish before returning.
            if getattr(self, "_has_spawn", False):
                builder.call(self.cg.f_scheduler_drain, [])
            self._emit_return(builder)

    def _emit_print(self, builder: ir.IRBuilder, stmt: PrintStmt) -> None:
        # Emit each part newline-free, then one trailing line feed; pure-literal print stays byte-identical to pre-interpolation behavior
        zero = ir.Constant(self.cg.i32, 0)
        ...
        for part in stmt.parts:
            if part.kind == "lit":
                ...
                # "var": interpolate bound string, no trailing newline
    ...
    def _emit_funcall_compose_arg(
        ...
        fname = stmt.name.lower()
        l_val = self._emit_operand(builder, stmt.left)
        r_val = self._emit_operand(builder, stmt.right)
        if fname in self.local_builtins:
    ...
    def _emit_funcall_decompose_ret(
        ...
        arg = self._emit_operand(builder, stmt.arg)
        if fname in self.local_builtins:
            # Builtins take two args; second slot receives ath_NULL
    ...
    def _emit_subscript(
        ...
        n_val = self._emit_operand(builder, stmt.index)
        result = builder.call(self.cg.f_index, [s_val, n_val])
        self._write_var(builder, stmt.target, result)

    def _emit_slice(self, builder: ir.IRBuilder, stmt: SliceStmt) -> None:
        s_val = self._read_var(builder, stmt.source)
        i_val = self._emit_operand(builder, stmt.start)
        j_val = self._emit_operand(builder, stmt.end)
        # Bundle indices into a composite the slice runtime decomposes, then install I and J as deps so killing either endpoint kills the slice on next observation
    ...
    def _emit_branch(self, builder: ir.IRBuilder, stmt: BranchStmt) -> None:
        ...
        # Consume V; re-read in case a body rebound V, matching a literal `V.DIE();` after the branch
