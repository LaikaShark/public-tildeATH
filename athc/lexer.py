...
# Human-readable token-kind names for parser diagnostics so users never see internal enum names
_TOKEN_DISPLAY = {
    TokenKind.SEMI: "';'",
    TokenKind.COMMA: "','",
    TokenKind.LBRACKET: "'['",
...
# Token kinds whose literal value is worth showing when reporting what was found
...
class Lexer:
    ...
    def _read_int(self, line: int, col: int) -> None:
        ...
        # '.' before digit opens fractional part, 'e'/'E' opens exponent; '..' stays DOTDOT (slice); '.' before non-digit stays '.DIE'. Either part makes FLOAT
        is_float = False
        ...
        if self._peek() in ("e", "E"):
            # Exponent needs >=1 digit (after optional sign); look ahead without consuming so `1exit` keeps `1` and `exit` separate
            off = 1
        ...
        if n < INT64_MIN or n > INT64_MAX:
            # Too big for int64: arbitrary-precision bignum literal
            self._emit(TokenKind.BIGINT, text, line, col)
            return
        self._emit(TokenKind.INT, text, line, col)

    # Escape tables: input-char -> decoded byte(s)
    _PRINT_ESCAPES = {
        ";": ";",
        "\\": "\\",
        "n": "\n",
    ...
    def _read_print_payload(self) -> None:
        ...
        def flush_literal() -> None:
            # Emit accumulated literal run as one RAWTEXT part
            if buf:
        ...
        while self.pos < len(self.src) and self._peek() != ";":
            ...
            if c == "\\":
                ...
                # `$NAME` interpolates runtime string bound to NAME; bare `$` must be `\$`
